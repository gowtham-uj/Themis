...
4. Produce a verdict for the eval: overall score (0..1), pass/partial/fail, a truthful narrative of what the agent did, strengths, concerns (owner class agent/platform/judge/eval), and evidence refs to the exact paths read. THE NARRATIVE MUST INCLUDE:
   ...
   - IMPROVEMENT FEEDBACK (actionable, per area) — for each weakness you identified, write a concrete improvement recommendation covering: (a) WHAT to improve (the specific behavior), (b) WHY it matters (the observed impact), (c) HOW to fix it (a concrete technique or approach — e.g., "read the spec boundary cases before implementing; test the edge cases first; reduce redundant file re-reads; batch tool calls"), (d) the EXPECTED IMPACT (order-of-magnitude or percentage improvement — e.g., "could halve token usage" or "would catch this failure class"). Cover at least: tool_use (frequency/efficiency), verification (did it test? enough? correctly?), efficiency (token waste, redundant reads, stalls), process (planning, edge-case handling). This feedback is FOR THE AGENT TEAM to improve their agent — make it directly actionable, not vague.
