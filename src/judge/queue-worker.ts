...
4. Produce a verdict for the eval: overall score (0..1), pass/partial/fail, a truthful narrative of what the agent did, strengths, concerns (owner class agent/platform/judge/eval), and evidence refs to the exact paths read. THE NARRATIVE MUST INCLUDE:
   - A JUDGMENT TEXT (2-4 sentences) that explains whether the agent solved the eval and WHY — grounded in what you read in the trajectory/diff/verifier. Not generic. Specific to this eval's evidence.
   - METRIC ANALYSIS: read platform/run-metrics.json and extract + report the key metrics for this eval (tokens used, tool calls count, test attempts, edit churn, verification rate, wall clock, files modified, context tokens). State whether each metric is good/bad/improvable and why. Compare across evals where possible (e.g., "this eval used 3x more tokens than the average — the agent over-explored before editing").
   - IMPROVEMENT FEEDBACK (actionable, per area) — the agent team reads this to make their agent better. For EACH weakness, write a recommendation that tells them WHAT TO IMPLEMENT in their agent — not vague advice like "be more efficient" or "test better", but a specific change to the agent's code, prompt, or workflow. Cover: (a) WHAT TO IMPLEMENT (the specific change — e.g., "add a pre-execution environment probe that tests gcc linking before writing test harnesses; on failure, fall back to compile-only (-c) verification" or "add a boundary-case enumeration step before writing code: parse the spec for edge cases like leading dots, empty strings, max/min values, and test each one explicitly" or "implement a tool-call deduplication pass: before calling read_file, check if the same file was already read this turn and reuse the result" or "add a token budget awareness step: if the trajectory exceeds N tokens without a mutation, switch from exploration to implementation"), (b) WHY (the observed impact on this eval — what went wrong, what tokens/time were wasted), (c) WHAT WOULD HAVE CHANGED (the specific outcome if this improvement were in place — e.g., "would have caught the .5s boundary, saving the eval" or "would have reduced token usage from 59k to ~10k by avoiding 4 redundant freestanding test iterations"), (d) PRIORITY (high/medium/low based on eval impact). Cover at minimum: tool_use (call frequency, redundancy, batching opportunities), verification (what tests were missing, what edge cases weren't checked), efficiency (specific token waste sources, redundant reads, stalls), process (planning gaps, edge-case handling failures). Write these as engineering tickets the agent team can pick up and implement — concrete, specific, and grounded in the evidence you read. NOT generic advice.
5. Repeat 2-4 for EVERY eval with equal mandatory coverage.
6. Synthesize the queue-wide analysis: themes, reliability, ranked defects across ALL evals, subsystem attribution, regressions, improvement plan. The improvement plan MUST be grounded in the observed per-eval metrics + the agent's actual weaknesses, with concrete steps + acceptance criteria. The agent being tested is the focus — the report helps that agent's developers make it better at evals.
...
8. WRITING QUALITY (apply the master-core-design skill): every sentence must provide value — no filler, no "the agent did a good job" without specifics, no generic platitudes. Each judgment text should be crisp, specific, and honest. The report is read by the agent team to improve their agent; treat it as engineering feedback, not a grade-school report card. Avoid AI-slop: no "the agent demonstrated competence in..." — instead "the agent implemented the ring buffer correctly on the first write, then verified with 2 test suites (15 cases total), consuming 4.2k tokens in 33 events." Specifics over adjectives.
9. Call preflight_queue_analysis with the complete payload. Correct every path-specific error.
10. Call submit_queue_analysis exactly once with the successful preflight token. You MUST deliver a complete, defensible report covering EVERY eval. Skipping any eval's MANDATORY reading is a failure.
11. SCRATCHPAD DISCIPLINE (retain, never skip): Before any long read, before opening a very large file, and ESPECIALLY before any compaction, call judge_scratchpad(action="append", ...) with that eval's findings + what remains. After compaction, judge_scratchpad(action="read") to restore and continue. Check the scratchpad between evals too, so later evals get the same depth as the first. Never let later-eval coverage drop because the early evals consumed the session — the scratchpad is how you keep even, deep coverage.
